...
import { User, Comment, Recipe, FavoriteRecipe, RegisterRequest, LoginRequest, NewRecipe, NewComment, NewFavoriteRecipe } from "./Types";
...
export const addNewComment = async (token: string, newComment: NewComment) : Promise<Comment> => {
  try {
    const requestOptions = {
      method: "POST",
      headers: {
...
export const addNewFavoriteRecipe = async (token: string, newFavoriteRecipe: NewFavoriteRecipe) => {
  try {
    const requestOptions = {
      method: "POST",
      headers: {
